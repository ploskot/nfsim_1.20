...
% experiment 6: query effect, get cause

clearvars -except m atleast

atleast= 1; % minimum occurrence
rxn_type= 2; % cluster to query
WW2= [5 10 15 20]; % query (effect) sub-sequence length
WW1= 1:15; % response (cause) sub-sequence length
...
fprintf('#rxn1=%d, #rxn2=%d, #rxn3=%d\n', ...
        length(rxn1),length(rxn2),length(rxn3));
...
for w2=1:nW2
  ...
  for w1=1:nW1
    ...
    xyn= yn1+yn2+yn12;
    if xn1==0, p1= '-'; PP(w1,1)= 0; else
      PP(w1,1)= 100*yn1/xyn; p1= sprintf('%.1f%%',PP(w1,1)); 
    end
    if xn2==0, p2= '-'; PP(w1,2)= 0; else
      PP(w1,2)= 100*yn2/xyn; p2= sprintf('%.1f%%',PP(w1,2));
    end
    if yn12==0, p3= '-'; PP(w1,3)= 0; else
      PP(w1,3)= 100*yn12/xyn; p3= sprintf('%.f%%',PP(w1,3));
    end
    %fprintf(' W1=%-2d n1=%-6d n2=%-5d %3d(%s)/%d %3d(%s)/%d %d(%s)\n', ...
    %        W1,n1,n2,yn1,p1,xn1,yn2,p2,xn2,yn12,p3);
    fprintf('.');
  end 
  fprintf('\n');
  ...
  if w2==3 || w2==4, xlabel('xx'); end %xlabel('W_1')
  if w2==1 || w2==3, ylabel('yy'); end % ylabel('%')
  title(sprintf('tt%d',w2)); % title(sprintf('W_2=%d',W2))
  xticklabels({'1','','','','5','','','','','10','','','','','15'})
end

% export figure
fname= sprintf('figs/pict2%d-%d%d.eps',m,rxn_type,atleast);
