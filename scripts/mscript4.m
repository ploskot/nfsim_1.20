...
% experiment 5: generate causal statements
...
rxn_type= 3;
W1= 25; % query (cause)
W2= 25; % response (effect)

Models= ['A' 'B' 'C' 'D' 'E'];
model= Models(m);
report('Model',model,': ',atleast, rxn_type, W1, W2)
...
jj2= find(Q1<atleast);  % at most

%n0= size(unique(dat2(jj1,ii1),'rows'),1);
n1= length(jj1);  
...
yNR1= setdiff(xNR1,yNR12,'rows'); % within query
yNR2= setdiff(xNR2,yNR12,'rows'); % outside query
