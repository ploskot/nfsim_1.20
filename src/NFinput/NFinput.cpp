...
{
    if (!verbose)
	cout << "reading xml file (" + filename + ")  \n\t[";
    if (verbose)
	cout << "\tTrying to read xml model specification file: \t\n'" <<
	    filename << "'" << endl;
	...
	if (!pModel) {
	    cout << "\tNo 'model' tag found.  Quitting.";
	    return NULL;
	}
	...
	if (!pModel->Attribute("id")) {
		...
		s = new System("nameless", false, globalMoleculeLimit);
	    else
		s = new System("nameless", true, globalMoleculeLimit);
	    if (verbose)
		cout <<
		    "\tNo System name given, so I'm calling your system: "
		    << s->getName() << endl;
	} else {
	    modelName = pModel->Attribute("id");
	    //We have to add complex bookkeeping if we are blocking same complex binding
	    if (!blockSameComplexBinding)
		s = new System(modelName, false, globalMoleculeLimit);
	    else
		s = new System(modelName, true, globalMoleculeLimit);
	    if (verbose)
		cout << "\tCreating system: " << s->getName() << endl;
	}

	// set evaluation of complex-scoped local functions (true or false)
	s->setEvaluateComplexScopedLocalFunctions
	    (evaluateComplexScopedLocalFunctions);
	...
	if (!pListOfParameters) {
	    cout << "\tNo 'ListOfParameters' tag found.  Quitting.";
	    delete s;
	    return NULL;
	}
	...
	if (!pListOfMoleculeTypes) {
	    cout << "\tNo 'ListOfMoleculeTypes' tag found.  Quitting.";
	    delete s;
	    return NULL;
	}
	...
	if (!pListOfSpecies) {
	    cout << "\tNo 'ListOfSpecies' tag found.  Quitting.";
	    delete s;
	    return NULL;
	}
	...
	if (!pListOfReactionRules) {
	    cout << "\tNo 'ListOfReactionRules' tag found.  Quitting.";
	    delete s;
	    return NULL;
	}
	...
	if (!pListOfObservables) {
	    cout << "\tNo 'ListOfObservables' tag found.  Quitting.";
	    delete s;
	    return NULL;
	}
	...
	if (!initParameters(pListOfParameters, s, parameter, verbose)) {
	    cout <<
		"\n\nI failed at parsing your Parameters.  Check standard error for a report."
		<< endl;
	    if (s != NULL)
		delete s;
	    return NULL;
	...
	if (!initMoleculeTypes
	    (pListOfMoleculeTypes, s, allowedStates, verbose)) {
	    cout <<
		"\n\nI failed at parsing your MoleculeTypes.  Check standard error for a report."
		<< endl;
	    if (s != NULL)
		delete s;
	    return NULL;
	...
	if (!initStartSpecies
	    (pListOfSpecies, s, parameter, allowedStates, verbose)) {
	    cout <<
		"\n\nI failed at parsing your species.  Check standard error for a report."
		<< endl;
	    if (s != NULL)
		delete s;
	    return NULL;
	...
	if (!initObservables
	    (pListOfObservables, s, parameter, allowedStates, verbose,
	     suggestedTraversalLimit)) {
	    cout <<
		"\n\nI failed at parsing your observables.  Check standard error for a report."
		<< endl;
	    if (s != NULL)
		delete s;
	    return NULL;
	...
	if (!verbose)
	    cout << "-";
	else if (pListOfFunctions)
	    cout << "\n\tReading list of Functions..." << endl;
	if (pListOfFunctions) {
	    if (!initFunctions
		(pListOfFunctions, s, parameter, pListOfObservables,
		 allowedStates, verbose)) {
		cout <<
		    "\n\nI failed at parsing your Global Functions.  Check standard error for a report."
		    << endl;
		if (s != NULL)
		    delete s;
		return NULL;
	...
	if (!initReactionRules
	    (pListOfReactionRules, s, parameter, allowedStates,
	     blockSameComplexBinding, verbose, suggestedTraversalLimit)) {
	    cout <<
		"\n\nI failed at parsing your reaction rules.  Check standard error for a report."
		<< endl;
	    if (s != NULL)
		delete s;
	    return NULL;
	...
	cout <<
	    "\nError reading the file.  I could not find / open it, or it is not valid xml."
	    << endl;
    }
...
bool NFinput::initParameters(TiXmlElement * pListOfParameters, System * s,
			     map < string, double >&parameter,
			     bool verbose)
{
    try {
	...
	for (pParamElement =
	     pListOfParameters->FirstChildElement("Parameter");
	     pParamElement != 0;
	     pParamElement =
	     pParamElement->NextSiblingElement("Parameter")) {

	    //Try to extract out the parameter name
	    string paramName;
	    if (!pParamElement->Attribute("id")) {
		cerr <<
		    "\t\t!!A Parameter is undefined! It is missing the 'id' attribute!  Quitting.\n";
		return false;
	    } else {
		...
		cerr << "\t\t!!A Parameter '" << paramName <<
		    "' does not have the 'value' attribute defined! Quitting.\n";
		return false;
	    } else {
		...
		cerr << "I couldn't understand your parameter: " <<
		    paramName << endl;
		cerr << e.what() << endl;
		return false;
	    }
		...
		cerr << "You tried to define the parameter '" + paramName +
		    "' more than once.  I think" << endl;
		cerr <<
		    "you made a mistake.  I'll forgive you, but I'm quitting now."
		    << endl;
		return false;
	    }
	    ...
	    if (verbose)
		cout << "\t\t Identified parameter:\t" << paramName <<
		    "\tValue:" << d << endl;
	}
	return true;
    }
    catch( ...) {
	cerr << "Undefined exception thrown while parsing the parameters."
	    << endl;
	return false;
    }
    return false;
				...
				bool verbose)
{
	...
	for (pMoTypeEl =
	     pListOfMoleculeTypes->FirstChildElement("MoleculeType");
	     pMoTypeEl != 0;
	     pMoTypeEl = pMoTypeEl->NextSiblingElement("MoleculeType")) {
	    //Check if MoleculeType tag has a name...
	    if (!pMoTypeEl->Attribute("id")) {
		cerr <<
		    "!!!Error:  MoleculeType tag must contain the id attribute.  Quitting."
		    << endl;
		return false;
	    }
	    ...
	    std::transform(typeName.begin(), typeName.end(),
			   lowerCaseTypeName.begin(),::tolower);
	    if (typeName.compare("null") == 0
		|| typeName.compare("trash") == 0) {
		if (verbose)
		    cout << "\t\tSkipping Moleculetype of name: '" +
			typeName + "'" << endl;
		continue;
	    }
	    if (verbose)
		cout << "\t\tReading and Creating MoleculeType: " +
		    typeName + "(";
		...
		if (popFlag.compare("1") == 0) {
		    isPopulation = true;
		    if (verbose)
			cout << "\t\tTreating molecule type '" << typeName
			    << "' as a population" << endl;
		}
	    }
	    //Get the list of components in the moleculeType
	    TiXmlElement *pListOfComp =
		pMoTypeEl->FirstChildElement("ListOfComponentTypes");
	    if (pListOfComp) {
		if (isPopulation) {	// Populations aren't allowed to have components!  --Justin
		    cerr << "!!!Error:  Population type " << typeName <<
			" is not allowed to have components! Quitting." <<
			endl;
		    return false;
		}
		...
		TiXmlElement *pComp;
		for (pComp =
		     pListOfComp->FirstChildElement("ComponentType");
		     pComp != 0;
		     pComp = pComp->NextSiblingElement("ComponentType")) {
		    //Check again for errors by making sure the component has a name
		    if (!pComp->Attribute("id")) {
			if (verbose)
			    cout << " ?? ...\n";
			cerr <<
			    "!!!Error:  ComponentType tag in MoleculeType: '"
			    + typeName +
			    "' must contain the id attribute.  Quitting."
			    << endl;
			return false;
		    }
		    ...
		    for (vector < string >::iterator it =
			 compLabels.begin(); it != compLabels.end();
			 it++, pos++) {
			...
			if ((*it) == compName) {
			    ...
			    for (unsigned int k = 0;
				 k < firstSymSiteToAppend.size(); k++) {
				if (firstSymSiteToAppend.at(k) == pos) {
				    shouldAdd = false;
				    break;
				}
			    }
			    if (shouldAdd)
				firstSymSiteToAppend.push_back(pos);


			    string newCompName = compName;
			    string num = "0";
			    bool matchedSiteName = false;
			    for (unsigned int is = 0;
				 is < identicalComponents.size(); is++) {
				if (identicalComponents.at(is).at(0) ==
				    (compName + "1")) {
				    unsigned int lastIndex =
					identicalComponents.at(is).size();
				    std::stringstream lastIndexStream;
				    lastIndexStream << lastIndex + 1;
				    num = lastIndexStream.str();
				    //cout<<"identified num = "<<num<<endl;
				    newCompName = compName + num;
				    identicalComponents.
					at(is).push_back(newCompName);
		    ...
		    TiXmlElement *pListOfAllowedStates =
			pComp->FirstChildElement("ListOfAllowedStates");
			...
			for (pAlStates =
			     pListOfAllowedStates->FirstChildElement
			     ("AllowedState"); pAlStates != 0;
			     pAlStates =
			     pAlStates->NextSiblingElement("AllowedState"))
			{
			    if (!pAlStates->Attribute("id")) {
				cerr <<
				    "!!!Error:  AllowedState tag in ComponentType '"
				    + compName + "' of MoleculeType: '" +
				    typeName +
				    "' must contain the id attribute.  Quitting."
				    << endl;
				return false;
			...
			if (stringComponentNameCount > 0) {
				...
				cerr <<
				    "Warning!! when creating Component '" +
				    compName + "' of MoleculeType: '" +
				    typeName +
				    ...
				    +
				    "strings used as component names!  All state values must be integers.  So be careful!"
				    << endl;
			    }
			    ...
			    for (pAlStates =
				 ...
				 ("AllowedState"); pAlStates != 0;
				 pAlStates =
				 pAlStates->NextSiblingElement
				 ("AllowedState")) {
				string aState = pAlStates->Attribute("id");
				if (allowedStates.find
				    (typeName + "_" + compName + "_" +
				     aState) != allowedStates.end())
				    continue;
				allowedStates[typeName + "_" + compName +
					      "_" + aState] =
				    allowedStateCount;
			...
			// If we should parse as integer states
			else if (intComponentNameCount > 0
				 && stringComponentNameCount == 0) {

			    //First make sure that all the state values were greater than zero
			    if (throwStateValLessThanZeroError
				|| maxStateValue < 0) {
				cerr << "Error when creating Component '" +
				    compName + "' of MoleculeType: '" +
				    typeName +
				...
				cerr << "Error when creating Component '" +
				    ...
				    +
				    " a value of 10,000!  Recheck your moleculeTypes."
				    << endl;
				return false;
			    }
			...
			defaultCompState.
			    push_back(possibleStateNames.at(0));
		    else
		...
		cout << ")" << endl;
	    if (verbose) {
		for (unsigned int is = 0; is < identicalComponents.size();
		     is++) {
		    cout << "\t\t\t-Identified Equivalent Components: ";
		    for (unsigned int isvec = 0;
			 isvec < identicalComponents.at(is).size();
			 isvec++) {
			cout << identicalComponents.
			    at(is).at(isvec) << " ";
		    } cout << endl;
		}
	    }
		...
		string originalCompLabel =
		    compLabels.at(firstSymSiteToAppend.at(k));
		compLabels.at(firstSymSiteToAppend.at(k)) =
		    compLabels.at(firstSymSiteToAppend.at(k)) + "1";

		string oldKeyStart =
		    typeName + "_" + originalCompLabel + "_";
		//cout<<":::"<<typeName+"_"+originalCompLabel+"1_"<<endl;
		map < string, int >::iterator it;
		for (it = allowedStates.begin(); it != allowedStates.end();
		     it++) {
		    string mappedKey = (*it).first;
		    if (mappedKey.size() <= oldKeyStart.size())
			continue;
		    //cout<<mappedKey.substr(0,50)<<"  "<<mappedKey.substr(0,oldKeyStart.size())<<endl;

		    if (mappedKey.substr(0, oldKeyStart.size()) ==
			oldKeyStart) {
			//cout<<"Found!! :"<<mappedKey.substr(0,oldKeyStart.size())<<endl;
			allowedStates[typeName + "_" + originalCompLabel +
				      "1_" +
				      mappedKey.
				      substr(oldKeyStart.size())] =
			    (*it).second;
	    ...
	    MoleculeType *mt =
		new MoleculeType(typeName, compLabels, defaultCompState,
				 possibleComponentStates,
				 isIntegerComponent, isPopulation, s);
	...
	cerr << "Caught some unknown error when creating MoleculeTypes." <<
	    endl;
	return false;
    }
			       ...
			       bool verbose)
{
	...
	TiXmlElement *pSpec;
	for (pSpec = pListOfSpecies->FirstChildElement("Species");
	     pSpec != 0; pSpec = pSpec->NextSiblingElement("Species")) {
	    //First get the species name and make sure it exists
	    string speciesName;
	    if (!pSpec->Attribute("id")) {
		cerr <<
		    "Species tag without a valid 'id' attribute.  Quiting"
		    << endl;
		return false;
	    } else {
		...
		cerr << "Species " << speciesName <<
		    " does not have a 'concentration' attribute.  Quitting"
		    << endl;
		return false;
	    } else {
		...
		try {
		    specCountInteger =
			(int) NFutil::convertToDouble(specCount);
			...
			if (parameter.find(specCount) == parameter.end()) {
			    cerr << "Could not find parameter: " <<
				specCount << " when creating species " <<
				speciesName << ". Quitting" << endl;
			    return false;
			}
			specCountInteger =
			    (int) parameter.find(specCount)->second;
		    }
		...
		cerr <<
		    "I cannot, in good conscience, make a negative number ("
		    << specCount << ") of species when creating species "
		    << speciesName << ". Quitting" << endl;
		return false;
	    }
		...
		pSpec->FirstChildElement("ListOfMolecules");
	    if (!pListOfMol) {
		cerr << "Species " << speciesName <<
		    " contains no molecules!  I think that was a mistake, on your part, so I'm done."
		    << endl;
		return false;
	    }
	    // Give our users a nice little message...
	    if (verbose)
		cout << "\t\tCreating " << specCountInteger <<
		    " instances of the Species: " << speciesName << endl;
	    ...
	    TiXmlElement *pMol;
	    for (pMol = pListOfMol->FirstChildElement("Molecule");
		 pMol != 0; pMol = pMol->NextSiblingElement("Molecule")) {
		// only one molecule per population species!!  --justin
		if (found_population) {
		    cerr <<
			"!!!Error.  More than one population molecule when creating species '"
			<< speciesName << "'. Quitting" << endl;
		    return false;
		}
		...
		if (!pMol->Attribute("name") || !pMol->Attribute("id")) {
		    cerr <<
			"!!!Error.  Invalid 'Molecule' tag found when creating species '"
			<< speciesName << "'. Quitting" << endl;
		    return false;
		} else {
		...
		if (molName == "Null" || molName == "NULL"
		    ...
		    if (verbose)
			cout <<
			    "\t\t\tSkipping a null molecule in species declaration"
			    << endl;
		    continue;
		}
		if (molName == "Trash" || molName == "TRASH"
		    || molName == "trash") {
		    if (verbose)
			cout <<
			    "\t\t\tSkipping a trash molecule in species declaration"
			    << endl;
		    continue;
		}
		...
		if (verbose)
		    cout << "\t\t\tIncluding Molecule of type: " << molName
		...
		if (found_population && found_particle) {
		    cerr <<
			"!!!Error.  Found mixed population and agent molecule types when creating species '"
			<< speciesName << "'. Quitting" << endl;
		    return false;
		}
		...
		TiXmlElement *pListOfComp =
		    pMol->FirstChildElement("ListOfComponents");
		if (pListOfComp) {
		    TiXmlElement *pComp;
		    for (pComp =
			 pListOfComp->FirstChildElement("Component");
			 pComp != 0;
			...
			if (!pComp->Attribute("id")
			    || !pComp->Attribute("name")
			    || !pComp->Attribute("numberOfBonds")) {
			    cerr <<
				"!!!Error.  Invalid 'Component' tag found when creating '"
				<< molUid << "' of species '" <<
				speciesName << "'. Quitting" << endl;
			    return false;
			} else {
			...
			if (mt->isEquivalentComponent(compName)) {
			    ...
			    int *eqCompClass;
			    int n_eqComp;
			    mt->getEquivalencyClass(eqCompClass, n_eqComp,
						    compName);


			    bool couldPlaceSymComp = false;
			    for (int eq = 0; eq < n_eqComp; eq++) {
				string eqCompNameToCompare =
				    mt->getComponentName(eqCompClass[eq]);
				//cout<<"comparing to: "<<eqCompNameToCompare<<endl;
				bool foundMatch = false;
				for (unsigned int ucn = 0;
				     ucn < usedComponentNames.size();
				     ucn++) {
				    if (usedComponentNames.
					at(ucn).compare
					(eqCompNameToCompare) == 0) {
					foundMatch = true;
					break;
				    }
				}
				...
				cout <<
				    "Too many symmetric sites specified, when creating species: "
				    << speciesName << endl;
				return false;
			    }
			} else {
			    for (unsigned int ucn = 0;
				 ucn < usedComponentNames.size(); ucn++) {
				if (usedComponentNames.
				    at(ucn).compare(compName) == 0) {
				    cout <<
					"Specified the same component multiple times, when creating species: "
					<< speciesName << endl;
				    return false;
				}
			    }
			...
			if (pComp->Attribute("state")) {
			    ...
			    if (allowedStates.find
				(molName + "_" + compName + "_" +
				 compStateValue) == allowedStates.end()) {
				cerr <<
				    "You are trying to create a molecule of type '"
				    << molName << "', but you gave an " <<
				    endl;
				cerr <<
				    "invalid state! The state you gave was: '"
				    << compStateValue <<
				    "'.  Quitting now." << endl;
				return false;
			    } else {
				...
				int stateValueInt =
				    allowedStates.find(molName + "_" +
						       compName + "_" +
						       compStateValue)->second;
			...
			cout <<
			    "!!! warning: no list of components specified for molecule: '"
			    << molUid << "' of species '" << speciesName <<
			    "'" << endl;
		    }
		}
		...
		if (!found_population) {
			...
			int k = 0;
			for (snIter = stateName.begin();
			     snIter != stateName.end(); k++, snIter++) {
	    ...
	    TiXmlElement *pListOfBonds =
		pListOfMol->NextSiblingElement("ListOfBonds");
	    if (pListOfBonds) {
		// Complain and quit if this is a population species (no bonds allowed!)
		//  --Justin, 4Mar2011
		if (found_population) {
		    cerr <<
			"!! Attempt to create illegal bond in population species: "
			<< speciesName << ".  Quitting." << endl;
		    return false;
		}
		//First get the information on the bonds in the complex
		TiXmlElement *pBond;
		for (pBond = pListOfBonds->FirstChildElement("Bond");
		     pBond != 0;
		     pBond = pBond->NextSiblingElement("Bond")) {
		    string bondId, bSite1, bSite2;
		    if (!pBond->Attribute("id")
			|| !pBond->Attribute("site1")
			|| !pBond->Attribute("site2")) {
			cerr << "!! Invalid Bond tag for species: " <<
			    speciesName << ".  Quitting." << endl;
			return false;
		    } else {
		    ...
		    try {
			string bSiteName1 =
			    bSiteSiteMapping.find(bSite1)->second;
			int bSiteMolIndex1 =
			    bSiteMolMapping.find(bSite1)->second;
			string bSiteName2 =
			    bSiteSiteMapping.find(bSite2)->second;
			int bSiteMolIndex2 =
			    bSiteMolMapping.find(bSite2)->second;

			for (int j = 0; j < specCountInteger; j++) {
			    Molecule::bind(molecules.
					   at(bSiteMolIndex1).at(j),
					   bSiteName1.c_str(),
					   molecules.
					   at(bSiteMolIndex2).at(j),
					   bSiteName2.c_str());
			}

		    } catch(exception & e) {
			cout << "!!!!Invalid site value for bond: '" <<
			    bondId << "' when creating species '" <<
			    speciesName << "'. Quitting" << endl;
			return false;
		    }
		}
	    ...
	    for (mIter = molecules.begin(); mIter != molecules.end();
		 mIter++) {
		(*mIter).clear();
	    }
				...
				bool verbose, int &suggestedTraversalLimit)
{
	...
	for (pRxnRule =
	     pListOfReactionRules->FirstChildElement("ReactionRule");
	     pRxnRule != 0;
	     pRxnRule = pRxnRule->NextSiblingElement("ReactionRule")) {
		...
		if (!pRxnRule->Attribute("id")) {
		    cerr <<
			"ReactionRule tag without a valid 'id' attribute.  Quiting"
			<< endl;
		    return false;
		} else {
		...
		if (verbose)
		    cout << "\t\tCreating Reaction Rule: " << rxnName <<
			endl;
		...
		if (pRxnRule->Attribute("symmetry_factor")) {
		    try {
			symmetryFactor =
			    NFutil::convertToDouble(pRxnRule->Attribute
						    ("symmetry_factor"));
			if (symmetryFactor <= 0.0)
			    throw std::runtime_error
				("Symmetry Factor must be a positive value!");
			useSymmetryFactor = true;
			if (verbose)
			    cout << "\t\t\tUsing symmetry factor = " <<
				symmetryFactor << endl;
		    }
		    catch(std::runtime_error & e1) {
			cerr << "Error!! Symmetry Factor for ReactionRule "
			    << rxnName <<
			    " was not set properly.  quitting." << endl;
			exit(1);
		    }
		}
		...
		TiXmlElement *pListOfReactantPatterns =
		    pRxnRule->FirstChildElement("ListOfReactantPatterns");
		if (!pListOfReactantPatterns) {
		    cout <<
			"!!!!!!!!!!!!!!!!!!!!!!!! Warning:: ReactionRule "
			<< rxnName << " contains no reactant patterns!" <<
			endl;
		    continue;
		}
		...
		for (pReactant =
		     pListOfReactantPatterns->FirstChildElement
		     ("ReactantPattern"); pReactant != 0;
		     pReactant =
		     pReactant->NextSiblingElement("ReactantPattern")) {
		    const char *reactantName = pReactant->Attribute("id");
		    if (!reactantName) {
			cerr << "Reactant tag in reaction " << rxnName <<
			    " without a valid 'id' attribute.  Quiting" <<
			    endl;
			return false;
		    }
		    if (verbose)
			cout << "\t\t\tReading Reactant Pattern: " <<
			    reactantName << endl;

		    TiXmlElement *pListOfMols =
			pReactant->FirstChildElement("ListOfMolecules");
		    if (pListOfMols) {
			TemplateMolecule *tm =
			    readPattern(pListOfMols, s, parameter,
					allowedStates, reactantName,
					reactants, comps, symMap, verbose,
					suggestedTraversalLimit);
			if (tm == NULL)
			    return false;
			templates.push_back(tm);
		    } else {
			cerr << "Reactant pattern " << reactantName <<
			    " in reaction " << rxnName <<
			    " without a valid 'ListOfMolecules'!  Quiting."
			    << endl;
			return false;
		    }
		}
		...
		TiXmlElement *pListOfOperations =
		    pRxnRule->FirstChildElement("ListOfOperations");
		if (!pListOfOperations) {
		    cout <<
			"!!!!!!!!!!!!!!!!!!!!!!!! Warning:: ReactionRule "
			<< rxnName <<
			" contains no operations!  This rule will do nothing!"
			<< endl;
		    continue;
		}
		...
		for (pAdd = pListOfOperations->FirstChildElement("Add");
			...
			cerr <<
			    "A specified Add operation in ReactionClass: '"
			    << rxnName << "' does not\n" <<
			    "have a valid id attribute.  Quitting." <<
			    endl;
			return false;
		    }
		    ...
		    for (unsigned int appIndex = 0;
			 appIndex < addedProductPatterns.size();
			 appIndex++) {
			if (addedProductPatterns.
			    at(appIndex).compare(id) == 0) {
		    ...
		    if (verbose)
			cout << "\t\t\t***Identified addition of product: "
			    + id + "." << endl;
			...
			cerr << "Error:: ReactionClass " << rxnName <<
			    " contains an Add transform\n" <<
			    "with id that does not refer to a molecule." <<
			    endl;
			return false;
		    }
			...
			pRxnRule->FirstChildElement
			("ListOfProductPatterns");
		    if (!pListOfProductPatterns) {
			cerr << "Error:: ReactionRule " << rxnName <<
			    " contains no product patterns,\n" <<
			    "but needs at least one to add a molecule creation rule!"
			    << endl;
			return false;
		    }
		    ...
		    for (pProduct =
			 pListOfProductPatterns->FirstChildElement
			 ("ProductPattern"); pProduct != 0;
			 pProduct =
			 pProduct->NextSiblingElement("ProductPattern")) {
			//First extract out the product Id
			string productName;
			if (!pProduct->Attribute("id")) {
			    cerr << "Product pattern in ReactionRule " +
				rxnName +
			...
			if (productName != patt_id)
			    continue;

			TiXmlElement *pListOfMols =
			    pProduct->FirstChildElement("ListOfMolecules");

			if (pListOfMols == NULL) {
			    cerr << "Reactant product pattern " <<
				productName << " in reaction\n" << rxnName
				<<
				" without a valid 'ListOfMolecules'!  Quiting."
				<< endl;
			    return false;
			...
			for (pMolecule =
			     pListOfMols->FirstChildElement("Molecule");
			     pMolecule != 0;
			     pMolecule =
			     pMolecule->NextSiblingElement("Molecule")) {
			    //First extract out the molecule Id
			    string moleculeName;
			    if (!pMolecule->Attribute("id")) {
				cerr << "Product molecule in ReactionRule "
				    << rxnName << " does not\n" <<
			    ...
			    bool ok =
				NFinput::readProductMolecule(pMolecule, s,
							     parameter,
							     allowedStates,
							     productName,
							     moleculeCreatorsList,
							     comps,
							     verbose);

			    if (!ok) {
				cerr <<
				    "Some problem reading product molecule for Add transform in reaction "
				    << rxnName << "." << endl;
				return false;
			    }
		    ...
		    if (!found_mol) {
			cerr << "Could not find product molecule '" << id
			    << "' referenced in\n" <<
			    "Add transform in reaction " << rxnName << "."
			    << endl;
			return false;
		    }
		}
		...
		if (moleculeCreatorsList.empty()) {
		    ...
		    {
			addmol_templates.
			    push_back((*mc_iter)->getTemplateMolecule());
		    }
		...
		for (pStateChange =
		     pListOfOperations->FirstChildElement("StateChange");
		     pStateChange != 0;
		     pStateChange =
		     pStateChange->NextSiblingElement("StateChange")) {
		    //Make sure all the information about the state change is here
		    string site, finalState;
		    if (!pStateChange->Attribute("site")
			|| !pStateChange->Attribute("finalState")) {
			cerr <<
			    "A specified state change operation in ReactionClass: '"
			    + rxnName + "' does not " << endl;
			cerr <<
			    "have a valid site or finalState attribute.  Quitting."
			    << endl;
			return false;
		    } else {
		    ...
		    component *c;
		    int finalStateInt = 0;
		    if (!lookup(c, site, comps, symMap))
			return false;

		    // Check if this is modifying a population (illegal!)
		    if (c->t->getMoleculeType()->isPopulationType()) {
			cerr <<
			    "Attempt to change state of a population type molecule in ReactionClass: '"
			    << rxnName << "'. Quitting." << endl;
			return false;
		    }
		    //handle both increment and decrement states first...
		    if (finalState == "PLUS") {
			if (!ts->addIncrementStateTransform(c->t,
							    c->symPermutationName))
			    return false;

			if (verbose) {
			    cout <<
				"\t\t\t***Identified increment state of site: "
				+ c->t->getMoleculeTypeName() + "(" +
				c->symPermutationName;
			    cout << ") to new state value: oldStateValue+1"
				<< endl;
			}
		    } else if (finalState == "MINUS") {
			if (!ts->addDecrementStateTransform(c->t,
							    c->symPermutationName))
			    return false;

			if (verbose) {
			    cout <<
				"\t\t\t***Identified decrement state of site: "
				+ c->t->getMoleculeTypeName() + "(" +
				c->symPermutationName;
			    cout << ") to new state value: oldStateValue-1"
				<< endl;
			...
			try {
			    ...
			    if (allowedStates.find
				(c->t->getMoleculeTypeName() + "_" +
				 lookupname + "_" + finalState) ==
				allowedStates.end()) {
				...
				cout <<
				    "Error! in NFinput, when looking up state: "
				    << c->t->getMoleculeTypeName() + "_" +
				    c->symPermutationName + "_" +
				    finalState << endl;
				cout <<
				    "Could not find this in the list of allowed states!  exiting!"
				    << endl;
				exit(1);
				//}
			    }
			    finalStateInt =
				allowedStates.find(c->
						   t->getMoleculeTypeName()
						   + "_" + lookupname +
						   "_" +
						   finalState)->second;
			    ...
			    cerr <<
				"Error in adding a state change operation in ReactionClass: '"
				+ rxnName + "'." << endl;
			    cerr <<
				"It seems that the final state is not valid."
				<< endl;
			    return false;
			}
			if (!ts->addStateChangeTransform(c->t,
							 c->
							 symPermutationName,
							 finalStateInt))
			    return false;
			if (verbose) {
			    cout <<
				"\t\t\t***Identified state change of site: "
				+ c->t->getMoleculeTypeName() + "(" +
				c->symPermutationName;
			    cout << ") to new state value: " +
				finalState << endl;
			}
		    }
		}
		...
		for (pDeleteBond =
		     pListOfOperations->FirstChildElement("DeleteBond");
		     pDeleteBond != 0;
		     pDeleteBond =
		     pDeleteBond->NextSiblingElement("DeleteBond")) {
		    //Make sure all the information about the unbinding operation change is here
		    string site1, site2;
		    if (!pDeleteBond->Attribute("site1")
			|| !pDeleteBond->Attribute("site2")) {
			cerr <<
			    "A specified binding operation in ReactionClass: '"
			    + rxnName + "' does not " << endl;
			cerr <<
			    "have a valid site1 or site2 attribute.  Quitting."
			    << endl;
			return false;
		    } else {
		    ...
		    component *c1;
		    component *c2;
		    if (!lookup(c1, site1, comps, symMap))
			return false;
		    if (!lookup(c2, site2, comps, symMap))
			return false;
		    ...
		    if (!ts->addUnbindingTransform(c1->t,
						   c1->symPermutationName,
						   c2->t,
						   c2->symPermutationName))
			return false;
		    if (verbose) {
			cout << "\t\t\t***Identified unbinding of site: " +
			    c1->t->getMoleculeTypeName() + "(" +
			    c1->symPermutationName + ")";
			cout << " to site " +
			    c2->t->getMoleculeTypeName() + "(" +
		...
		for (pAddBond =
		     pListOfOperations->FirstChildElement("AddBond");
		     pAddBond != 0;
		     pAddBond = pAddBond->NextSiblingElement("AddBond")) {
		    //cout<<"adding binding transform!"<<endl;
		    //Make sure all the information about the binding operation is here
		    string site1, site2;
		    if (!pAddBond->Attribute("site1")
			|| !pAddBond->Attribute("site2")) {
			cerr <<
			    "A specified binding operation in ReactionClass: '"
			    << rxnName << "' does not\n" <<
			    "have a valid site1 or site2 attribute.  Quitting."
		    ...
		    string::size_type PPpos1 = site1.find("_PP");
		    string::size_type PPpos2 = site2.find("_PP");
			...
			cerr <<
			    "Attempt to AddBond at site on a population type molecule in ReactionClass: '"
			    << rxnName << "'. Quitting." << endl;
			return false;
		    }
		    ...
		    string reactantNum1 = site1.substr(0,
						       site1.
						       find_first_of("_",
								     underScore1
								     + 1));
		    string reactantNum2 = site2.substr(0,
						       site2.
						       find_first_of("_",
								     underScore2
								     + 1));
			...
			if (!ts->addNewMoleculeBindingTransform(c1->t,
								c1->symPermutationName,
								c2->t,
								c2->symPermutationName))
			    return false;
		    } else {
			if (!ts->addBindingTransform(c1->t,
						     c1->
						     symPermutationName,
						     c2->t,
						     c2->
		    ...
		    if (verbose) {
			cout << "\t\t\t***Identified binding of site: " <<
			    c1->t->getMoleculeTypeName()
			    << "(" << c1->symPermutationName << ")" <<
			    " to site " << c2->t->getMoleculeTypeName()
			    << "(" << c2->symPermutationName << ")" <<
			    endl;
			if (isNewMoleculeBond)
			    cout <<
				"\t\t\t   (this bond connects a new molecule that was synthesized by this rule)"
				<< endl;
		    }
		}
		...
		for (pDelete =
		     pListOfOperations->FirstChildElement("Delete");
		     pDelete != 0;
		     pDelete = pDelete->NextSiblingElement("Delete")) {
		    //Make sure all the information about the state change is here
		    string id, delMolKeyword;
		    if (!pDelete->Attribute("id")
			|| !pDelete->Attribute("DeleteMolecules")) {
			cerr <<
			    "A specified delete operation in ReactionClass: '"
			    + rxnName + "' does not " << endl;
			cerr <<
			    "have a valid id attribute or a DeleteMolecules attribute.  Quitting."
			    << endl;
			return false;
		    } else {
			try {
				...
				if (verbose) {
				    cout <<
					"\t\t\t***Identified deletion of a molecule: "
					+ id << ". ";
				    if (delMolKeyword.compare("0") == 0) {
					cout <<
					    "\n\t\t\t\tDeleteMolecules keyword is turned off."
					    << endl;
				    } else {
					cout <<
					    "\n\t\t\t\tDeleteMolecules keyword is turned on, so only this specific molecule will be removed."
					    << endl;
				    }
				}
				...
				if (delMolKeyword.compare("0") == 0) {
				    ...
				    cout <<
					"No delete molecule keyword, so delete this molecule, conditional on"
					"the fact that it cannot create multiple additional species."
					<< endl;

				    //Give a warning, because the behavior of this function is very strange indeed!!!
				    cerr <<
					"\nERROR!  You created a reaction ("
					+ rxnName +
					") that deletes a molecule, but you did not use"
					<< endl;
				    cerr <<
					...
					if (!ts->addDeleteMolecule(c.t,
								   TransformationFactory::DECREMENT_POPULATION))
					    return false;
				    } else {
					if (!ts->addDeleteMolecule(c.t,
				...
				if (verbose) {
				    cout <<
					"\t\t\t***Identified deletion of the complete pattern: "
					+ id << ". ";
				    if (delMolKeyword.compare("0") == 0) {
					cout <<
					...
					cout <<
					    "\n\t\t\t\tDeleteMolecules keyword is turned on, so only the molecules in the pattern will be removed."
					    << endl;
				    }
				}
				// get component
				...
				if (delMolKeyword.compare("0") == 0) {
				    ...
				    component c = comps.find(id)->second;
				    if (c.t->getMoleculeType()->isPopulationType()) {	// we're dealing with a population here, create a decrement population transform
					if (!ts->addDecrementPopulation
					    (c.t))
					    return false;
				    } else {	// we're dealing with a particle here, create regular delete transform
					if (!ts->addDeleteMolecule(c.t,
					...
					"\nERROR!  You created a reaction ("
					+ rxnName +
					") that deletes a molecule, and you did use"
					<< endl;
				    cerr <<
					"the 'DeleteMolecules' keyword.  Unfortunately, BNG provided NFsim with information about"
					<< endl;
				    cerr <<
					"deleting an entire species.  This error should never happen, but if it does, contact us"
					<< endl;
				    cerr <<
					"through the NFsim or BioNetGen websites to report this error.  "
					<< endl;
				...
				cerr <<
				    "Error in adding an delete molecule operation in ReactionClass: '"
				    + rxnName + "'." << endl;

				cerr << "Invalid read!  Looking for " << id
				    << " but could not find it!" << endl;
				exit(1);
			    }
				...
				+ rxnName + "'." << endl;
			    cerr <<
				"It seems that I couldn't find the molecule to delete that you are refering to. (I was looking for ID: "
				<< pDelete->Attribute("id") << endl;
			    return false;
			}
		    }
		...
		TiXmlElement *pRateLaw =
		    pRxnRule->FirstChildElement("RateLaw");
		if (!pRateLaw) {
		    cerr << "!!Error:: ReactionRule " << rxnName <<
			" contains no rate law specification!" << endl;
		    return false;
		}

		if (!pRateLaw->Attribute("totalrate")) {
		    cerr <<
			"\n!!Error! This XML file was generated using an older version of BioNetGen that does not support the 'TotalRate' convention!"
			<< endl;
		    cerr <<
			"You should upgrade your BioNetGen distribution now, or download the latest NFsim package, and regenerate this XML file."
			<< endl;
		} else {
		    try {
			int rf =
			    NFutil::
			    convertToInt(pRateLaw->Attribute("totalrate"));
			if (rf > 0)
			    totalRateFlag = true;
			if (verbose)
			    cout << "\t\t\tTotal rate flag = " <<
				totalRateFlag << endl;
		    }
		    catch(std::runtime_error & e1) {
			//cerr<<e1.what()<<endl;
			cerr << "Error!! totalrate flag for ReactionRule "
			    << rxnName <<
			    " was not set properly.  quitting." << endl;
			exit(1);
		    }
		...
		if (!pRateLaw->Attribute("id")
		    || !pRateLaw->Attribute("type")) {
		    cerr << "!!Error:: ReactionRule " << rxnName <<
			" rate law specification: cannot read 'id' or 'type' attribute!"
			<< endl;
		    return false;
		} else {
			...
			TiXmlElement *pListOfRateConstants =
			    pRateLaw->FirstChildElement
			    ("ListOfRateConstants");
			if (!pListOfRateConstants) {
			    cerr << "Elementry Rate Law definition for " <<
				rxnName <<
				" does not have listOfRateConstants specified!  Quiting"
				<< endl;
			    return false;
			}
			TiXmlElement *pRateConstant =
			    pListOfRateConstants->FirstChildElement
			    ("RateConstant");
			if (!pRateConstant) {
			    cerr << "Elementry Rate Law definition for " <<
				rxnName <<
				" does not have RateConstants specified!  Quiting"
				<< endl;
			    return false;
			} else {
				...
				cerr <<
				    "Elementry Rate Law definition for " <<
				    rxnName <<
				    " does not have a valid RateConstant value!  Quiting"
				    << endl;
				return false;
			    } else {
				rateValue =
				    pRateConstant->Attribute("value");
			    }

			    //Try to parse it into a double value or look it up in the parameter map
			    double rate = 0;
			    bool usedParam = false;
			    string rateValueParameterName = "";
			    try {
				rate = NFutil::convertToDouble(rateValue);
			    } catch(std::runtime_error & e1) {
				if (parameter.find(rateValue) ==
				    parameter.end()) {
				    cerr << "Could not find parameter: " <<
					rateValue <<
					" when reading rate for rxn " <<
					rxnName << ". Quitting" << endl;
				    return false;
				}
			...
			pRateConstant =
			    pRateConstant->NextSiblingElement
			    ("RateConstant");
			if (pRateConstant != NULL) {
			    cout <<
				"\n\n!!Warning:: Multiple RateConstant tags present for RateLaw definition of "
				<< rxnName << "." << endl;
			    cout <<
				"Only the first RateConstant given will be used..."
				<< endl;
			}
			...
			if (!pRateLaw->Attribute("name")) {
			    cerr << "!!Error:: ReactionRule " << rxnName <<
				" rate law specification Function: cannot read 'name' attribute!"
				<< endl;
			    return false;
			}
			...
			TiXmlElement *pListOfArgs =
			    pRateLaw->FirstChildElement("ListOfArguments");
			if (pListOfArgs) {
			    TiXmlElement *pArg;
			    for (pArg =
				 pListOfArgs->FirstChildElement
				 ("Argument"); pArg != 0;
				 pArg =
				 pArg->NextSiblingElement("Argument")) {
				if (!pArg->Attribute("id")
				    || !pArg->Attribute("type")
				    || !pArg->Attribute("value")) {
				    cerr << "!!Error:: ReactionRule " <<
					rxnName <<
					" rate law specification Function: Argument tag \n";
				    cerr <<
					"must have id, type and value attributes defined!"
					<< endl;
				    return false;
				}
				...
				if (argValue.find("_M") != string::npos) {
				    ...
				    if (reactants.find(argValue) !=
					reactants.end()) {
					// found good molecule reference!
					ts->addLocalFunctionReference
					    (reactants.
					     find(argValue)->second, argId,
					     LocalFunction::MOLECULE);
					if (verbose) {
					    cout << "\t\t\t\tScope of tag "
						<< argId <<
						" is MOLECULE (argValue="
						<< argValue << ")" << endl;
					}
				    } else {
					cerr << "!!Error:: ReactionRule "
					    << rxnName <<
					    " rate law specification Function:\n"
					    << " cannot find referent " <<
					    argId <<
					    " for local function tag " <<
					    argValue << "." << endl;
					return false;
				    }
				} else if (argValue.find("_RP") !=
					   string::npos) {
				    // this appears to be a pattern ("species") reference
				    // ..make sure the pattern reference is good
				    if (comps.find(argValue) !=
					comps.end()) {
					// found good pattern reference!
					component c =
					    comps.find(argValue)->second;
					ts->addLocalFunctionReference(c.t,
								      argId,
			...
			if (isGlobal) {
			    string functionName =
				pRateLaw->Attribute("name");
			    GlobalFunction *gf =
				s->getGlobalFunctionByName(functionName);
			    if (gf != NULL) {
				ts->finalize();
				...
				CompositeFunction *cf =
				    s->getCompositeFunctionByName
				    (functionName);
				if (cf == NULL) {
				    cerr << "When parsing reaction: '" <<
					rxnName <<
					"', could not identify function: '"
					<< functionName << "' in\n";
				    cerr <<
					"the system.  Therefore, I must abort."
					<< endl;
				    return false;
				}
				ts->finalize();
				r = new FunctionalRxnClass(rxnName, cf, ts,
							   s);
			    }
			} else {
				...
				pRateLaw->Attribute("name");
			    LocalFunction *lf =
				s->getLocalFunctionByName(functionName);
			    if (lf != NULL) {
				cout <<
				...
				r = new DORRxnClass(rxnName, 1, "", ts, cf,
						    funcArgs, s);
			    }
			}
			...
			TiXmlElement *pListOfArgs1 =
			    pRateLaw->FirstChildElement
			    ("ListOfArguments1");
			if (pListOfArgs1) {
			    TiXmlElement *pArg;
			    for (pArg =
				 pListOfArgs1->FirstChildElement
				 ("Argument"); pArg != 0;
				 pArg =
				 pArg->NextSiblingElement("Argument")) {
				if (!pArg->Attribute("id")
				...
				if (reactants.find(argValue) !=
				    reactants.end()) {
				    ts->addLocalFunctionReference
					(reactants.find(argValue)->second,
					 argId, LocalFunction::MOLECULE);
				    if (verbose) {
					cout << "\t\t\t\tScope is MOLECULE"
					    << endl;
				    }
				}
			    }
			...
			TiXmlElement *pListOfArgs2 =
			    pRateLaw->FirstChildElement
			    ("ListOfArguments2");
			if (pListOfArgs2) {
			    TiXmlElement *pArg;
			    for (pArg =
				 pListOfArgs2->FirstChildElement
				 ("Argument"); pArg != 0;
				 pArg =
				 pArg->NextSiblingElement("Argument")) {
				if (!pArg->Attribute("id")
			...
			if (funcArgs1.size() < 1) {
			    cout <<
				"Error!! FunctionProduct ratelaw is missing argument for function1."
				<< endl;
			    exit(1);
			}
			...
			if (funcArgs2.size() < 1) {
			    cout <<
				"Error!! FunctionProduct ratelaw is missing argument for function2."
				<< endl;
			    exit(1);
			}
			...
			string functionName1 =
			    pRateLaw->Attribute("name1");
			LocalFunction *lf1 =
			    s->getLocalFunctionByName(functionName1);
			if (lf1 != NULL) {
			    cout <<
				"Error!! call a local function through a composite function always!"
				<< endl;
			...
			string functionName2 =
			    pRateLaw->Attribute("name2");
			LocalFunction *lf2 =
			    s->getLocalFunctionByName(functionName2);
			if (lf2 != NULL) {
			    cout <<
				"Error!! call a local function through a composite function always!"
				<< endl;
			...
			CompositeFunction *cf1 =
			    s->getCompositeFunctionByName(functionName1);
			CompositeFunction *cf2 =
			    s->getCompositeFunctionByName(functionName2);

			r = new DOR2RxnClass(rxnName, 1, "", ts, cf1, cf2,
					     funcArgs1, funcArgs2, s);
			...
			if (!pListOfRateConstants) {
			    cerr <<
				"Michaelis-Menten Rate Law definition for "
				<< rxnName <<
				" does not have a ListOfRateConstants tag!  Quiting"
				<< endl;
			    return false;
			} else {
			    double kcat = 0;
			    ...
			    if (!pRateConstant) {
				cerr <<
				    "Michaelis-Menten Law definition for "
				    << rxnName <<
				    " does not have kcat value defined!  Quiting"
				    << endl;
				return false;
			    } else {
				//Get the rate constant value
				...
				if (!pRateConstant->Attribute("value")) {
					...
					"Michaelis-Menten Rate Law definition for "
					<< rxnName <<
					" does not have a valid kcat RateConstant 'value'!  Quiting"
					<< endl;
				    return false;
				} else {
				    kcatName =
				    ...
				    try {
					kcat =
					    NFutil::convertToDouble
					    (kcatName);
				    } catch(std::runtime_error & e1) {
					if (parameter.find(kcatName) ==
					    parameter.end()) {
					    cerr <<
						"Could not find parameter: "
						<< kcatName <<
						" when reading kcat rate for rxn "
						<< rxnName << ". Quitting"
						<< endl;
					    return false;
					}
					kcat =
					    parameter.
					    find(kcatName)->second;
					usedParam = true;
				    }
				    if (verbose) {
					cout <<
					    "\t\t\t\t...setting kcat rate to be: "
					    << kcatName;
					if (usedParam)
					    cout << " (has value: " << kcat
						<< ")";
				...
				cerr <<
				    "Michaelis-Menten Law definition for "
				    << rxnName <<
				    " does not have Km value defined!  Quiting"
				    << endl;
				return false;
			    } else {
				//Get the rate constant value
				...
				if (!pRateConstant->Attribute("value")) {
					...
					"Michaelis-Menten Rate Law definition for "
					<< rxnName <<
					" does not have a valid Km RateConstant 'value'!  Quiting"
					<< endl;
				    return false;
				} else {
				    KmName =
				    ...
				    bool usedParam = false;
				    try {
					Km = NFutil::convertToDouble
					    (KmName);
				    } catch(std::runtime_error & e1) {
					if (parameter.find(KmName) ==
					    parameter.end()) {
					    cerr <<
						"Could not find parameter: "
						<< KmName <<
						" when reading Km rate for rxn "
						<< rxnName << ". Quitting"
						<< endl;
					    return false;
					}
			...
			cerr <<
			    "!! Nfsim cannot, and will not, interpret a Rate Law of 'type': 'Sat' as BioNetGen.\n";
			cerr <<
			    "  once could.  You should instead use a Michaelis-Menten rate law (use type 'MM')\n";
			cerr <<
			    "  that also takes the parameters (kcat, Km) but is more accurate.\n"
			    << endl;
			...
			cerr <<
			    "!! I cannot yet interpret a Rate Law of 'type': "
			    << rateLawType << ".\n";
			cerr <<
			    "  Remember, an elementry reaction needs to be specified as 'Ele' (case sensitive),"
			    << endl;
			cerr <<
			    "  a Michaelis-Menten reaction should be specified as 'MM', and"
			    << endl;
			cerr <<
			    "  a functional reaction should be specified as 'Functional'."
			    << endl;
			cerr << "Quitting." << endl;
			//      return false;
		    }
...
bool NFinput::readObservableForTemplateMolecules(TiXmlElement * pObs,
						 ...
						 map < string,
						 int >&allowedStates,
						 int obsType, bool verbose,
						 int
						 &suggestedTraversalLimit)
{


    TiXmlElement *pListOfPatterns =
	pObs->FirstChildElement("ListOfPatterns");
    if (!pListOfPatterns) {
	cout << "\n\n!! Warning:: Observable " << observableName <<
	    " contains no patterns!" << endl;
	return true;
    }
    ...
    for (pPattern = pListOfPatterns->FirstChildElement("Pattern");
	 pPattern != 0;
	 pPattern = pPattern->NextSiblingElement("Pattern")) {
	const char *patternName = pPattern->Attribute("id");
	if (!patternName) {
	    cerr << "Pattern tag in observable " << observableName <<
		" without a valid 'id' attribute.  Quiting" << endl;
	    return false;
	}
	...
	if (pPattern->Attribute("relation")) {
		...
		try {
		    quantity =
			NFutil::
			convertToInt(pPattern->Attribute("quantity"));
		}
		catch(std::runtime_error & e) {
		    cerr <<
			"I couldn't parse the 'quantity' value when creating pattern: "
			<< patternName << endl;
		    cerr << e.what() << endl;
		    return false;
		}
	    } else {
		cerr << "Error when creating observable: " <<
		    observableName <<
		    ": relation attribute for stoichiometric\n";
		cerr <<
		    "observable was found without a matching quantity attribute."
		    << endl;
		return false;
	    }
	}
	...
	TiXmlElement *pListOfMols =
	    pPattern->FirstChildElement("ListOfMolecules");
	if (pListOfMols) {
		...
		if (!readPatternForSymmetry
		    (pListOfMols, s, patternName, comps, symComps,
		     verbose))
		    return false;

		//Get the permutations that were created from the symmetries
		vector < map < string, component > >permutations;
		if (!generateRxnPermutations
		    (permutations, symComps, symComps, verbose))
		    return false;
		...
		for (unsigned int p = 0; p < permutations.size(); p++) {
		    ...
		    TemplateMolecule *tm =
			readPattern(pListOfMols, s, parameter,
				    allowedStates, patternName,
				    allTemplatesMap, comps, symMap,
			...
			cerr << "Error when creating observable: " <<
			    observableName <<
			    ": a stoichiometric observable found for\n";
			cerr <<
			    "observable of type Molecules.  Currently, NFsim only handles stoichiometric Species\n";
			cerr <<
			    "observables for effeciency.  Rewrite this observable as type 'Species'."
			    << endl;
			return false;
		    }
		}
		...
		TemplateMolecule *tm =
		    readPattern(pListOfMols, s, parameter, allowedStates,
				patternName, allTemplatesMap, comps,
				symMap, verbose, suggestedTraversalLimit);
		if (tm == NULL)
		    return false;
		tmList.push_back(tm);
		...
		if (verbose && !relation.empty()) {
		    cout << "\t\t\t\t\tHas stoichiometric constraint: '" <<
			relation << " " << quantity << "'" << endl;
		}
	    }
	    ...
	    cerr << "Observable pattern in observable " << observableName
		<< " without a valid 'ListOfMolecules'!  Quiting." << endl;
	    return false;
	}
    }
			      ...
			      bool verbose, int &suggestedTraversalLimit)
{
	...
	for (pObs = pListOfObservables->FirstChildElement("Observable");
	    ...
	    if (!pObs->Attribute("id") || !pObs->Attribute("name")
		|| !pObs->Attribute("type")) {
		cerr <<
		    "Observable tag without a valid 'id' attribute.  Quiting"
		    << endl;
		return false;
	    } else {
	    ...
	    if (verbose)
		cout << "\t\tCreating Observable: '" << observableName <<
		    "' of type: '" << observableType << "'" << endl;
		...
		if (!readObservableForTemplateMolecules
		    (pObs, observableName, tmList, stochRelation,
		     stochQuantity, s, parameter, allowedStates,
		     Observable::MOLECULES, verbose,
		     suggestedTraversalLimit)) {
		    return false;
		}
		//Create the observable, which in this case, is a MoleculesObservable
		MoleculesObservable *mo =
		    new MoleculesObservable(observableName, tmList);
		...
		for (unsigned int k = 0; k < tmList.size(); k++) {
		    ...
		    for (unsigned int mtc = 0; mtc < addedMolTypes.size();
			 mtc++) {
			if (addedMolTypes.at(mtc) ==
			    tmList.at(k)->getMoleculeType()) {
			    alreadyAdded = true;
			    break;
			}
		    }
		    ...
		    addedMolTypes.push_back(tmList.
					    at(k)->getMoleculeType());
		}
		...
		if (!s->isUsingComplex()) {
		    cerr <<
			"You have not turned on complex book keeping, so I can not run you simulation"
			<< endl;
		    cerr <<
			"because your simulation has a Species observable."
			<< endl;
		...
		if (!readObservableForTemplateMolecules
		    (pObs, observableName, tmList, stochRelation,
		     stochQuantity, s, parameter, allowedStates,
		     Observable::SPECIES, verbose,
		     suggestedTraversalLimit)) {
		    return false;
		}

		SpeciesObservable *so =
		    new SpeciesObservable(observableName, tmList,
					  stochRelation, stochQuantity);
		s->addObservableForOutput(so);

	    } else {
		cerr << "Cannot create an Observable of type '" <<
		    observableType << "'." << endl;
		cerr <<
		    "The only valid types in NFsim are: 'Molecules' and 'Species', case sensitive."
		    << endl;
		return false;
	    }
				       ...
				       int &suggestedTraversalLimit)
{
	...
	TiXmlElement *pMol;
	for (pMol = pListOfMol->FirstChildElement("Molecule"); pMol != 0;
	     pMol = pMol->NextSiblingElement("Molecule")) {
	    //First get the type of molecule and retrieve the moleculeType object from the system
	    string molName, molUid;
	    if (!pMol->Attribute("name") || !pMol->Attribute("id")) {
		cerr <<
		    "!!!Error.  Invalid 'Molecule' tag found when creating pattern '"
		...
		|| molName == "null") {
		cerr <<
		    "\n\nError!  You cannot include a 'null' molecule in any reactant or observable pattern!"
		    << endl;
		cerr <<
		    "Null is a keyword in NFsim used for degradation, so cannot be used.  Check your"
		    << endl;
		cerr <<
		    "rules and patterns and remove any usage of 'Null' or 'null' or 'NULL'."
		    << endl;
		exit(1);
	    }
	    if (molName == "Trash" || molName == "TRASH"
		|| molName == "trash") {
		cerr <<
		    "\n\nError!  You cannot include a 'trash' molecule in any reactant or observable pattern!"
		    << endl;
		cerr <<
		    "Trash is a keyword in NFsim used for degradation, so cannot be used.  Check your"
		    << endl;
		cerr <<
		    "rules and patterns and remove any usage of 'Trash' or 'trash' or 'TRASH'."
		    << endl;
		exit(1);
	    }
...
//                                      cout<<compId<<endl;
		    ...
		    if (symMap.find(compId) == symMap.end()
			...
			if (pComp->Attribute("state")) {
				...
				if (allowedStates.find
				    ...
				    allowedStates.end()) {
				    cerr <<
					"You are trying to give a pattern of type '"
					<< molName << "', but you gave an "
					<< endl;
				    cerr <<
					"invalid state! The state you gave was: '"
			...
			int bondConstraint =
			    TemplateMolecule::NO_CONSTRAINT;
			if (pComp->Attribute("numberOfBonds")) {
				...
				try {
				    numOfBondsInt =
					NFutil::convertToInt(numOfBonds);
				}
				catch(std::runtime_error & e) {
				    cerr <<
					"I couldn't parse the numberOfBonds value when creating pattern: "
					<< patternName << endl;
				    cerr << e.what() << endl;
				    return NULL;
				}
				...
				if (numOfBondsInt == 0) {
				    bondConstraint =
					TemplateMolecule::EMPTY;
				} else if (numOfBondsInt == 1) {
				    bondConstraint =
					TemplateMolecule::OCCUPIED;
				    bSiteSiteMapping[compId] = compName;
				    bSiteMolMapping[compId] =
					tMolecules.size();
				} else {
				    cerr <<
					"I can only handle a site that has 0 or 1 bonds in pattern: "
					<< patternName << endl;
				    cerr << "You gave me " << numOfBondsInt
					<< " instead for component " <<
					compName << endl;
				    return NULL;
				}
			    }
			}
			tempmol->addSymCompConstraint(compName, compId,
						      bondConstraint,
						      stateConstraint);
			...
			if (pComp->Attribute("state")) {
				...
				if (allowedStates.find
				    ...
				    if (!lookup
					(symC, compId, comps, symMap)) {
					cerr <<
					    "Could not find the symmetric component when creating a component state, but there\n";
					cerr <<
					    "should have been one!!  I don't know what to do, so I'll quit."
					    << endl;
					return NULL;
				    }
				    stateName.
					push_back
					(symC->symPermutationName);
				    stateValue.push_back(stateValueInt);
				}
			    }
			}
			...
			if (pComp->Attribute("numberOfBonds")) {
				...
				cerr <<
				    "Could not find the symmetric component when creating a binding site, but there\n";
				cerr <<
				    "should have been one!!  I don't know what to do, so I'll quit."
				    << endl;
				return NULL;
				...
				occupiedBondSite.
				    push_back(symC->symPermutationName);
				...
				cerr <<
				    ...
				    << patternName << endl;
				cerr << "You gave me " << numOfBondsInt <<
				    " instead for component " << compName
				    << endl;
				return NULL;
			    }
			}
	    ...
	    int k = 0;
	    for (strVecIter = stateName.begin();
		 strVecIter != stateName.end(); k++, strVecIter++) {
		tempmol->addComponentConstraint(*strVecIter,
						(int) stateValue.at(k));
	    }
	    for (strVecIter = emptyBondSite.begin();
		 strVecIter != emptyBondSite.end(); strVecIter++) {
		tempmol->addEmptyComponent(*strVecIter);
	    }
	    for (strVecIter = occupiedBondSite.begin();
		 strVecIter != occupiedBondSite.end(); strVecIter++) {
	    ...
	    templates.insert(pair < string,
			     TemplateMolecule * >(molUid, tempmol));
	...
	if (pListOfBonds) {
	    ...
	    TiXmlElement *pBond;
	    for (pBond = pListOfBonds->FirstChildElement("Bond");
		 pBond != 0; pBond = pBond->NextSiblingElement("Bond")) {
		//First, grab the bond information that we need to set things up
		string bondId, bSite1, bSite2;
		if (!pBond->Attribute("id") || !pBond->Attribute("site1")
		    || !pBond->Attribute("site2")) {
		    cerr << "!! Invalid Bond tag for pattern: " <<
			patternName << ".  Quitting." << endl;
		    return NULL;
		} else {
		    bondId = pBond->Attribute("id");
...
//                                              cout << iter->first << '\t' << iter->second << '\n';
		    ...
		    if (bSiteSiteMapping.find(bSite1) !=
			bSiteSiteMapping.end()
			&& bSiteMolMapping.find(bSite1) !=
			bSiteMolMapping.end()
			&& bSiteSiteMapping.find(bSite2) !=
			bSiteSiteMapping.end()
			&& bSiteMolMapping.find(bSite2) !=
			bSiteMolMapping.end()
			) {
			...
			int bSiteMolIndex2 =
			    bSiteMolMapping.find(bSite2)->second;
			TemplateMolecule::
			    bind(tMolecules.at(bSiteMolIndex1),
				 bSiteName1.c_str(), bSite1,
				 tMolecules.at(bSiteMolIndex2),
				 bSiteName2.c_str(), bSite2);
			...
			cerr << "here" << endl;
			cerr << "!!!!Invalid site value for bond: '" <<
			    bondId << "' when creating pattern '" <<
			    patternName << "'. " << endl;
			cerr <<
			    "This may be caused because you are adding two bonds to one binding site or because you listed"
			    << endl;
			cerr <<
			    "a binding site at the end of the pattern that does not exist.  Quitting"
			    << endl;
			return NULL;
		    }
		}
		    ...
		    cerr << "!!!!Invalid site value for bond: '" << bondId
			<< "' when creating pattern '" << patternName <<
			"'. " << endl;
		    cerr <<
			"This may be caused because you are adding two bonds to one binding site or because you listed"
			<< endl;
	...
	try {
	    ...
	    for (strMapit = bSiteSiteMapping.begin();
		 strMapit != bSiteSiteMapping.end(); strMapit++) {
		...
		if (tMolecules.at(bSiteMolIndex)->
		    getMoleculeType()->isEquivalentComponent(bSiteName.
							     c_str())) {
		    continue;
		}
	    ...
	    cerr << "Something went wacky when I was parsing pattern '" <<
		patternName << "'." << endl;
	    cerr <<
		"It happened as I was trying to add an occupied binding site to a template molecule."
		<< endl;
	    return NULL;
	}
	...
	int setCount =
	    TemplateMolecule::getNumDisjointSets(tMolecules, sets,
						 uniqueSetId);
...
//              }
	...
	if (setCount > 1) {
		...
		for (unsigned int i = 0; i < tMolecules.size(); i++) {
		    if (uniqueSetId.at(i) == cSet) {
			tm1 = i;
			break;
		    }
		}
		for (unsigned int i = 0; i < tMolecules.size(); i++) {
		    if (uniqueSetId.at(i) == (cSet + 1)) {
			tm2 = i;
			break;
		    }
		}
		...
		tMolecules.at(tm1)->addConnectedTo(tMolecules.at(tm2),
						   ctIndex2);
		tMolecules.at(tm2)->addConnectedTo(tMolecules.at(tm1),
						   ctIndex1);
	    }
	...
	if (tMolecules.empty()) {
	    cerr << "You have a pattern named " << patternName <<
		" that doesn't include any actual patterns!  (Or I just couldn't find any)"
		<< endl;
	    cerr <<
		"Therefore, I see no other choice than to quit until you fix the problem."
		<< endl;
	    return NULL;
	}
	...
	if (verbose) {
	    cout << "\t\t\t\t => Final processed pattern: " <<
		finalTemplate->getPatternString() << endl;
	}
...
bool NFinput::readProductPattern(TiXmlElement * pListOfMol,
				 System * s, map < string,
				 double >&parameter, map < string,
				 int >&allowedStates, string patternName,
				 vector <
				 MoleculeType * >&productMoleculeTypes,
				 vector < vector <
				 int > >&stateInformation,
				 vector < vector <
				 int > >&bindingSiteInformation,
				 bool verbose)
{
	...
	const int bSiteIndex = 1, partnerMolTypeIndex =
	    2, partnerBsiteIndex = 3;
	...
	for (pMol = pListOfMol->FirstChildElement("Molecule"); pMol != 0;
	    ...
	    if (!pMol->Attribute("name") || !pMol->Attribute("id")) {
		cerr <<
		    "!!!Error.  Invalid 'Molecule' tag found when creating product pattern '"
		    << patternName << "'. Quitting" << endl;
		return false;
	    } else {
		molName = pMol->Attribute("name");
		...
		if (verbose)
		    cout <<
			"\t\t\t\tskipping a null molecule in product pattern..."
			<< endl;
		foundTrash = true;
		continue;
	    }
		...
		if (verbose)
		    cout <<
			"\t\t\t\tskipping a trash molecule in product pattern..."
			<< endl;
		foundTrash = true;
		continue;
	    }
	    ...
	    uniqueIdMapping.insert(pair < string,
				   int >(molUid,
					 currentMoleculeTypeIndex));

	    if (verbose)
		cout << "\t\t\t\tIncluding Product Molecule of type: " <<
		    molName << " with local id: " << molUid << endl;
		...
		for (pComp = pListOfComp->FirstChildElement("Component");
			...
			if (allowedStates.find
				...
				allowedStates.find(molName + "_" +
						   ...
						   compStateValue)->second;
			    stateInformation.
				at(proMolTypeIndex).push_back
				(currentMoleculeTypeIndex);
			    stateInformation.
				at(stateIndex).push_back
				(moltype->getCompIndexFromName(compName));
			    stateInformation.
				at(stateValue).push_back(stateValueInt);
			}
		    }
	...
	if (pListOfBonds) {
		...
		try {
			...
			at(proMolTypeIndex).push_back(bSiteMolTypeIndex1);
		    unsigned int bSiteIndex1 =
			productMoleculeTypes.
			at(bSiteMolTypeIndex1)->getCompIndexFromName
			(bSiteName1);
		    bindingSiteInformation.
			at(bSiteIndex).push_back(bSiteIndex1);
		    bindingSiteInformation.
			at(partnerMolTypeIndex).push_back(bSiteMolIndex2);
		    unsigned int bSiteIndex2 =
			productMoleculeTypes.
			at(bSiteMolIndex2)->getCompIndexFromName
			(bSiteName2);
		    bindingSiteInformation.
			at(partnerBsiteIndex).push_back(bSiteIndex2);
	...
	if (productMoleculeTypes.empty()) {
		...
		if (verbose)
		    cout <<
			"\t\t\t\tWarning: You have an add molecule rule, but only a Trash or Null pattern listed..."
			<< endl;
		return true;
	    }
	    cerr << "You have a product pattern named " << patternName <<
		" that doesn't include any actual patterns!  (Or I just couldn't find any)"
		<< endl;
	    cerr <<
		"Therefore, I see no other choice than to quit until you fix the problem."
...
bool NFinput::readProductMolecule(TiXmlElement * pMol,
				  System * s,
				  map < string, double >&parameter,
				  map < string, int >&allowedStates,
				  string patternName,
				  vector <
				  MoleculeCreator * >&moleculeCreatorsList,
				  map < string, component > &comps,
				  bool verbose)
{
	...
	if (!pMol->Attribute("name") || !pMol->Attribute("id")) {
	    cerr <<
		"!!!Error.  Invalid 'Molecule' tag found when creating product molecule '"
		<< patternName << "'. Quitting" << endl;
	    return false;
	}
	...
	if (molName == "Null" || molName == "NULL" || molName == "null") {
	    if (verbose)
		cout << "\t\t\t\treadProductMolecule is ignoring " <<
		    molName << " molecule..." << endl;
	    return true;
	}
	//Skip anything that is trash molecule
	...
	for (int k = 0; k < moltype->getNumOfComponents(); k++) {
		...
		string genericCompName =
		    moltype->getEquivalenceClassComponentNameFromComponentIndex
		    (k);
	...
	if (pListOfComp) {
		...
		if (moltype->isEquivalentComponent(compName)) {
		    ...
		    int eqClass =
			moltype->getEquivalencyClassNumber(compName);
		    //cout<<"class: "<<moltype->getEquivalencyClassNumber(compName)<<endl;
		    if (symmetricSitesNotUsed.at(eqClass).size() > 0) {
			compName =
			    (symmetricSitesNotUsed.at(eqClass)).front();
			(symmetricSitesNotUsed.at(eqClass)).pop();
		    } else {
			cerr <<
			    "In NFinput::readProductMolecule(...): some symmetric components were specified more times than they were originally declared!"
			    << endl;
			cerr <<
			    "This must be an error in BioNetGen, as all molecules to be created must have all component sites specified!\n\n"
			    << endl;
			return false;
		    }
		}
		...
		if (pComp->Attribute("state")) {
		    ...
		    //Make sure the given state is allowed
		    if (allowedStates.find(molName + "_" + compName + "_" +
					   compStateValue) ==
			allowedStates.end()) {
			cerr <<
			    "Attempt to assign invalid component state value '"
			    << compStateValue << "' in product molecule '"
			    << molName << "'.  Quitting now." << endl;
			return false;
		    }
		    ...
		    stateValueIndex =
			allowedStates.find(molName + "_" + compName + "_" +
					   compStateValue)->second;
		    // Push onto component states vector
		    component_states.push_back(pair < int,
					       int >(componentIndex,
						     stateValueIndex));

		    // add component state value constraint to the template molecule
		    tempmol->addComponentConstraint(compName,
						    compStateValue);
		}
	...
	for (unsigned int i = 0; i < symmetricSitesNotUsed.size(); i++) {
		...
		cerr <<
		    "In NFinput::readProductMolecule(...): some symmetric components exist, but were not defined!"
		    << endl;
		cerr <<
		    "This must be an error in BioNetGen, as all molecules to be created must have all component sites specified!\n\n"
		    << endl;
	...
	moleculeCreatorsList.push_back(new
				       MoleculeCreator(tempmol, moltype,
						       component_states));
