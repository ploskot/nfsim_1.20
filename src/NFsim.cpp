      ...
      if(!filename.empty())
	{
	  ...
	  if(s!=NULL)
		...
		if(s->isOutputtingBinary()) {
	      ...
	      // LQI: Register the output file for event history
	      if (argMap.find("eh")!=argMap.end()) {
		s->setEventHistory1();
		s->registerOutputFileLocation1(s->getName()+"_eh.gdat");
		if(verbose) cout<<"\tEvent history will be written to: "<< s->getName()+"_eh.gdat" <<endl<<endl;
	      }

	      // LQI: Register the output binary file for event history
	      if (argMap.find("ehb")!=argMap.end()) {
		s->setEventHistoryBin1();
		s->registerOutputFileLocation1(s->getName()+"_eh.dat");
		if(verbose) cout<<"\tEvent history will be written to: "<< s->getName()+"_eh.dat" <<endl<<endl;
	      }
...
bool runFromArgs(System *s, map<string,string> argMap, bool verbose)
{
  ...
  // LQI: print reactions in the simulation
  cout<<endl;
  s->printAllReactions();
...
void printHelp(string version)
{
  ...
  cout<<""<<endl;
  cout<<"  -eh               flag to request the recording of the events history into a csv file"<<endl;
  cout<<""<<endl;
  cout<<"  -ehb              flag to request the recording of the events history into a binary file"<<endl;
  cout<<""<<endl;	
