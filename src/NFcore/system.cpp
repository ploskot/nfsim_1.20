...
System::System(string name)
{
    ...
    useBinaryOutput1 = false;	// LQI
    useEventHistory1 = false;	// LQI
...
void
 System::setOutputToBinary()
{
    this->useBinaryOutput = true;
    if (outputFileStream.is_open()) {
	outputFileStream.close();
	cerr <<
	    "Error!! You are trying to switch the output of this system to Binary, but\n";
	cerr <<
	    "you already have an open file stream that is not binary!  The results are\n";
	cerr <<
	    "therefore unpredictable.  It would be better if you fix this problem first.\n";
	cerr <<
	    "This problem is caused when you call 'setOutputToBinary()' after you call\n";
...
void System::setEventHistory1()	// LQI
{
    this->useEventHistory1 = true;
    if (outputFileStream1.is_open()) {
	outputFileStream1.close();
	cerr <<
	    "Error in using event history file. File already opened.\n";
	cerr << "So I'm just going to stop now." << endl;
	exit(1);
    }
}

void System::setEventHistoryBin1()	// LQI
{
    this->useBinaryOutput1 = true;
    this->useEventHistory1 = true;
    if (outputFileStream1.is_open()) {
	outputFileStream1.close();
	cerr <<
...
void System::turnOff_OnTheFlyObs()
{
    ...
    for (rxnIter = allReactions.begin(); rxnIter != allReactions.end();
	 rxnIter++)
...
void System::registerOutputFileLocation(string filename)
{
	...
	outputFileStream.open((filename).c_str(),
			      ...
			      binary | ios_base::trunc);

	if (!outputFileStream.is_open()) {
	    cerr << "Error in System!  cannot open output stream to file "
		<< filename << ". " << endl;
	    cerr << "quitting." << endl;
	    exit(1);
	}
	...
	headerFile << "#\tTime";
	tabCount++;
	for (molTypeIter = allMoleculeTypes.begin();
	     molTypeIter != allMoleculeTypes.end(); molTypeIter++) {
	    int oTot = (*molTypeIter)->getNumOfMolObs();
	    for (int o = 0; o < oTot; o++) {
		headerFile << "\t" << (*molTypeIter)->
		    getMolObs(o)->getName();
		tabCount++;
	    }
	}
	if (outputGlobalFunctionValues)
	    for (functionIter = globalFunctions.begin();
		 functionIter != globalFunctions.end(); functionIter++) {
		headerFile << "\t" << (*functionIter)->getNiceName();
		tabCount++;
	    }
	if (outputEventCounter)
	    headerFile << "\tEventCounter";
	headerFile << endl;
...
// LQI
void System::registerOutputFileLocation1(string filename)
{
    if (outputFileStream1.is_open()) {
	outputFileStream1.close();
    }
    if (useBinaryOutput1) {
	outputFileStream1.open((filename).c_str(),
			       ios_base::
			       out | ios_base::binary | ios_base::trunc);
	...
	outputFileStream1.setf(ios::scientific);
	outputFileStream1.precision(8);
    }
}
...
void System::tagReaction(int rID)
{
    ...
    if (rID < 0 || (unsigned int) rID >= this->allReactions.size()) {
	cerr << "!!! Error when trying to tag reaction with reaction ID "
	    << rID << endl;
...
bool System::addGlobalFunction(GlobalFunction * gf)
{
    for (functionIter = globalFunctions.begin();
	 functionIter != globalFunctions.end(); functionIter++)
	if (gf->getName() == (*functionIter)->getName())
	    return false;
...
MoleculeType *System::getMoleculeTypeByName(string mName)
{
    ...
    cerr << "!!! warning !!! cannot find molecule type name '" << mName <<
	"' in System: '" << this->name << "'" << endl;
    exit(1);
    return 0;
}
...
Molecule *System::getMoleculeByUid(int uid)
{
    ...
    cerr << "!!! warning !!! cannot find active molecule with unique ID '"
	<< uid << "' in System: '" << this->name << "'" << endl;
    return 0;
}
...
{
    return allMoleculeTypes.
	at(moleculeTypeIndex)->getMolObsCount(observableIndex);
}
...
void System::prepareForSimulation()
{
    ...
    int match = 0;
    for (obsIter = speciesObservables.begin();
	 obsIter != speciesObservables.end(); obsIter++)
	(*obsIter)->clear();
...
double System::sim(double duration, long int sampleTimes, bool verbose)
{
    ...
    // LQI: event history buffer
    const int bSize = 5000;
    unsigned int i1, bPt = 0;
    unsigned int bCount[bSize], bFireCount[bSize], bRxnId[bSize];
    double bAval[bSize], bTime[bSize];
    string bRxnName[bSize];

    // LQI: header for event history
    unsigned int blockCounter = 0;
    if (this->isOutputtingEventHistory1() && !this->isOutputtingBinary1()) {
	outputFileStream1 << "# Block,Time,FireCount,A_val,RxnId,RxnName" << endl;
    }
    ...
    double delta_t = 0;
    unsigned long long iteration = 0, stepIteration = 0;
	...
	if (a_tot > ATOT_TOLERANCE)
	    delta_t = -log(NFutil::RANDOM_OPEN()) / a_tot;
	else {
	    delta_t = 0;
	...
	if ((current_time + delta_t) >= curSampleTime) {
	  ...
	  cout << "\tCPU time (sec): " << ((double) (clock() - start) / (double) CLOCKS_PER_SEC) << "s";
	  cout << "\t events (step): " << stepIteration << endl;
	  // LQi: increment block counter
	  if (this->isOutputtingEventHistory1()) { blockCounter++; }
	  stepIteration = 0;
	  recompute_A_tot();
	}
	...
	// LQI: Write event history into file
	if (this->isOutputtingEventHistory1()) {
	    bTime[bPt] = current_time;
	    bCount[bPt] = blockCounter;
	    bFireCount[bPt] = nextReaction->getFireCounter();
	    bAval[bPt] = nextReaction->get_a();
	    bRxnId[bPt] = nextReaction->getRxnId();
	    bRxnName[bPt] = nextReaction->getName();
	    bPt++;
	    if (bPt == bSize) {
	      if (this->isOutputtingBinary1()) {
		for (i1 = 0; i1 < bSize; i1++) {
		  outputFileStream1.write((char *) &bCount[i1], sizeof(unsigned int));		  
		  outputFileStream1.write((char *) &bTime[i1], sizeof(double));
		  outputFileStream1.write((char *) &bFireCount[i1], sizeof(unsigned int));
		  outputFileStream1.write((char *) &bAval[i1], sizeof(double));
		  outputFileStream1.write((char *) &bRxnId[i1], sizeof(unsigned int));
		  outputFileStream1.write((char *) &bRxnName[i1], bRxnName[i1].size());
		}
	      } else {
		for (i1 = 0; i1 < bSize; i1++) {
		  outputFileStream1 << bCount[i1] << "," << bTime[i1] << "," << bFireCount[i1] << "," << bAval[i1] << "," << bRxnId[i1] << "," << bRxnName[i1] << endl;
		}
	      }
	      if (verbose) cout << "Written " << bPt << " reaction events to file." << endl;
	      bPt = 0;
	    }
	}
    }
    ...
    if (bPt && this->isOutputtingEventHistory1()) { // flush event history buffer
      bPt--;
      if (this->isOutputtingBinary1()) {
	for (i1 = 0; i1 < bPt; i1++) {
	  outputFileStream1.write((char *) &bCount[i1], sizeof(unsigned int));		  
	...
	for (i1 = 0; i1 < bPt; i1++) {
      ...
      if (verbose) cout << "Added " << bPt << " reaction events to file." << endl;
    }
...
double System::stepTo(double stoppingTime)
{
	...
	if (a_tot > ATOT_TOLERANCE)
	    ...
	    cout <<
		"Total propensity is zero, no further rxns can fire in this step."
		<< endl;
	    break;
	}
...
void System::singleStep()
{
    ...
    recompute_A_tot();
    cout << "  -total propensity (a_total) calculated as: " << a_tot <<
	endl;
    if (a_tot > ATOT_TOLERANCE)
	delta_t = -log(NFutil::RANDOM_CLOSED()) / a_tot;
    else {
	//Otherwise, we can't react for the rest of this step
	delta_t = 0;
	cout << "  -Total propensity is zero, no further rxns can fire." <<
	    endl;
	return;
    }
...
void System::equilibrate(double duration, int statusReports)
{
	...
	eTime += stepLength;
	cout << "Equilibration has now elapsed for: " << eTime <<
	    " seconds." << endl;
    }
...
void System::outputAllObservableNames()
{
	...
	if (!csvFormat) {
	    ...
	    if (outputGlobalFunctionValues)
		for (functionIter = globalFunctions.begin();
		     functionIter != globalFunctions.end();
		     functionIter++) {
	...
	cout <<
	    "Warning: You cannot output observable names when outputting in Binary Mode."
	    << endl;
    }
}
				       ...
				       int eventCounter)
{
	...
	for (obsIter = obsToOutput.begin(); obsIter != obsToOutput.end();
	     obsIter++) {
	    (*obsIter)->clear();
	}
	...
	if (!csvFormat) {
		...
		for (functionIter = globalFunctions.begin();
		     functionIter != globalFunctions.end(); functionIter++)
		    outputFileStream << "  " <<
			FuncFactory::Eval((*functionIter)->p);
	    if (outputEventCounter) {
...
bool System::saveSpecies(string filename)
{
	...
	cerr <<
	    "Error in System when calling System::saveSpecies(string)!  Cannot open output stream to file "
	    << filename << ". " << endl;
	cerr << "quitting." << endl;
	exit(1);
    }
	...
	for (int j = 0; j < mt->getMoleculeCount(); j++) {
	    ...
	    if (reportedMolecules.find(mt->getMolecule(j)->getUniqueID())
		!= reportedMolecules.end()) {
		continue;
	    }
	    ...
	    molecules.clear();
	    mt->getMolecule(j)->traverseBondedNeighborhood(molecules,
							   ReactionClass::NO_LIMIT);
		...
		Molecule *m = (*iter);
		reportedMolecules.insert(pair < int,
					 bool >(m->getUniqueID(), true));

		//Fist, output the molecule name
		if (isFirst) {
		    speciesString += m->getMoleculeTypeName() + "(";
		    isFirst = false;
		} else {
		    speciesString += "." + m->getMoleculeTypeName() + "(";
		}
		...
		for (int s = 0;
		     s < m->getMoleculeType()->getNumOfComponents(); s++) {
		    // output the component name
		    string compName =
			m->getMoleculeType()->getComponentName(s);
		    if (m->getMoleculeType()->isEquivalentComponent(s)) {
			// symmetric site, so we need to look up its sym name
			compName =
			    m->
			    getMoleculeType
			    ()->getEquivalenceClassComponentNameFromComponentIndex
			    (s);
		    }
		    if (s == 0)
			speciesString += compName;
		    else
			speciesString += "," + compName;
			...
			speciesString +=
			    "~" +
			    m->getMoleculeType()->getComponentStateName(s,
									m->getComponentState
									(s));
		    }
			...
			int partnerID =
			    m->getBondedMolecule(s)->getUniqueID();
			int partnerSite =
			    m->getBondedMoleculeBindingSiteIndex(s);
			...
			if (partnerID < m->getUniqueID()) {
			    key->at(0) = partnerID;
			    key->at(1) = partnerSite;
			    key->at(2) = m->getUniqueID();
			    key->at(3) = s;
			} else {
			    key->at(2) = partnerID;
			    key->at(3) = partnerSite;
			    key->at(0) = m->getUniqueID();
			    key->at(1) = s;
			}

			//search if that key was already inserted
			bool foundExistingBond = false;
			for (unsigned int bnmIndex = 0;
			     bnmIndex < bondNumberMap.size(); bnmIndex++) {
			    if (key->at(0) ==
				bondNumberMap.at(bnmIndex)->at(0)
				&& key->at(1) ==
				bondNumberMap.at(bnmIndex)->at(1)
				&& key->at(2) ==
				bondNumberMap.at(bnmIndex)->at(2)
				&& key->at(3) ==
				bondNumberMap.at(bnmIndex)->at(3)) {
				thisBondNumber = bnmIndex + 1;
				foundExistingBond = true;
				if (debugOut)
				    cout << "Found bond number: " <<
					thisBondNumber << endl;
				break;
			    }
			}
			...
			if (!foundExistingBond) {
			    ...
			    if (debugOut)
				cout << "Creating bond number: " <<
				    bondNumberMap.size() << endl;
			}
			speciesString +=
			    "!" + NFutil::toString(thisBondNumber);
		    }
	    ...
	    if (reportedSpecies.find(speciesString) !=
		reportedSpecies.end()) {
		reportedSpecies[speciesString] =
		    reportedSpecies[speciesString] +
		    mt->getMolecule(j)->getPopulation();
	    } else {
		reportedSpecies.insert(pair < string,
				       int >(speciesString,
					     mt->
					     getMolecule(j)->getPopulation
					     ()));
	    }
		...
		vector < int >*v =
		    bondNumberMap.at(bondNumberMap.size() - 1);
    ...
    speciesFile << "# nfsim generated species list for system: '" <<
	this->name << "'\n";
    speciesFile << "# warning! this feature is not yet fully tested! \n";
    for (map < string, int >::iterator it = reportedSpecies.begin();
	 it != reportedSpecies.end(); it++)
...
// LQI: output reactions fired in the simulation
// assume text output *_eh.gdat
void System::printAllReactions()
{
  unsigned int N;
  recompute_A_tot();
  outputFileStream1 <<"#end" <<endl<<endl<< "# Reactions fired in the simulation:" <<endl;
  
  for (rxnIter = allReactions.begin(); rxnIter != allReactions.end(); rxnIter++) {
    if((*rxnIter)->getFireCnt1()==0) continue;
    outputFileStream1 << (*rxnIter)->getName() << ":  id="<<(*rxnIter)->getRxnId()<<", baseRate="<< (*rxnIter)->getBaseRate() <<", a=" << (*rxnIter)->get_a();
    outputFileStream1 << ", fired=" << (*rxnIter)->getFireCnt1() << "x" << endl;
    N = (*rxnIter)->getNumOfReactants();
    for(unsigned int r=0; r<N; r++) {
      outputFileStream1 << "      -[" << (*rxnIter)->getReactantCount(r)<< "]\t" << (*rxnIter)->getRxnTemplate1(r) << endl;
    }
    if(N==0) outputFileStream1 << "      >No Reactants: so this rule either creates new species or does nothing." <<endl; 
    outputFileStream1 << endl;
  }
}
...
void System::evaluateAllLocalFunctions()
{
	...
	for (int m = 0; m < (*molTypeIter)->getMoleculeCount(); m++) {
		...
		mol->traverseBondedNeighborhood(molList,
						ReactionClass::NO_LIMIT);
		...
		for (unsigned int l = 0; l < localFunctions.size(); l++) {
		    ...
		    /* double val = */
		    localFunctions.at(l)->evaluateOn(mol,
		...
		for (molListIter = molList.begin();
		     molListIter != molList.end(); molListIter++) {
...
Observable *System::getObservableByName(string obsName)
{
    ...
    cout.flush();
    cerr << "!!Warning, the system could not identify the observable: " <<
	obsName << ".\n";
    cerr <<
	"The calling function might catch this, or your program might crash now."
	<< endl;
    return 0;
}
...
void System::setParameter(string name, double value)
{
	...
	cout << "Warning! System parameter: '" << name <<
	    "' does not exist and will not be updated." << endl;
	return;
    }
    this->paramMap[name] = value;
}
...
{
    if (paramMap.size() == 0)
	cout << "no system parameters to print." << endl;
    else
	cout << "List of all system parameters:" << endl;
...
void System::outputAllPropensities(double time, int rxnFired)
{
	...
	for (unsigned int r = 0; r < allReactions.size(); r++) {
	    ...
	    for (int rl = 0; rl < allReactions[r]->getNumOfReactants();
		 rl++) {
...
NFstream & System::getOutputFileStream1()
{
    return outputFileStream1;
}

// friend functions
template < class T > NFstream & operator<<(NFstream & nfstream,
					   const T & value)
{
