...
vector<job*> parseJobsFile (string buffer) {
	...
	bool inBlock = false; // LQI
	int currentJobID = -1; // LQI
	...
	for (int m=0; m < int(lines->size()); m++) {
		...
		for (int i=0; i < int(strings->size()); i++) {
			...
			} else if ((long int)(*strings)[i].find("=") != -1) {
...
//     System* s = NFinput::initializeFromXML(jnow.filename, true, true);
	...
	for (int i = 0; (unsigned) i < jnow.argument.size(); ++i) {
		argMap[jnow.argument[i]] = jnow.argval[i];
	}
	for (int i = 0; (unsigned) i < jnow.parameters.size(); ++i) {
	...
	if (rank == 0) {
		...
		while (left > 0) {
			if (!done && slave_available) {
				if (jobQueue.size() <= (unsigned) jcount) {
					done = true;
				}
		...
		while (1) {
			...
			for (int i = 0; (unsigned) i < slave_filenames.size(); ++i) {
